<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE usuarios  set email = ?, password = ?, nombre = ?, telefono = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($email,$hashedPassword,$nombre,$telefono,$id));
            Database::disconnect();
            header("Location: index.php");
        }
    } else {
...
<head>
    <meta charset="utf-8">
	<title>CRUD - Actualizar Usuario</title>
